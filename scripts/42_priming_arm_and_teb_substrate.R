...
notes <- c(
  ...
  "falls ~50% at constant transcript, and the transcriptome rescales at the rate script 44",
  "fits (collapse_module_ownership.rds $defs$global_rate_fitted) with no",
  ...
  "mitochondrial compartment, so a uniform dose scaling (script 44's fitted global rate,",
  "$defs$global_rate_fitted) CANCELS in it. The content",
  ...
  sprintf(paste(" - PART B control passed: matched pairs retain %.3f (ctrl_median). Read it",
                "against script 44's fitted global rate ($defs$global_rate_fitted), not against",
                "the hard-coded GLOBAL_RATE, which is a reference copied from script 40."),
          ctrl_median),
