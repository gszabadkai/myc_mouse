...
# the legend's "does not survive correction across the 26", asserted
stopifnot(identical(mg$gene[1:2], c("Foxo3", "Bbc3")),
          min(1, mg$int_p[1] * nrow(mg)) > 0.05)
...
LEGEND <- panel_legend(
  ...
  detail = c(
    ...
    sprintf("AND FOXO3 IS WHY THE MIDDLE FACET IS THERE. Of the %d biogenesis- and cell-death-related genes script 44 curates, Foxo3 separates the genotypes more sharply than any (interaction %+.3f, nominal p %.4f); Bbc3 is second (%.4f), with a clear gap to the third (%s, %.3f). Foxo3 was not pre-specified, and %.4f does not survive correction across the %d (Bonferroni %.2f), so it is reported as exploratory. That is the ranking the text quotes, and it is a far smaller and more meaningful universe than the 8,774-gene scan Fig. 2H uses.",
            nrow(mg), int_de["lfc", "Foxo3"], mg$int_p[1], mg$int_p[2],
            mg$gene[3], mg$int_p[3], mg$int_p[1], nrow(mg),
            min(1, mg$int_p[1] * nrow(mg))),
