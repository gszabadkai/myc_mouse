...
# the archived main pipeline's (archive_main_pipeline/02_deseq_interaction_model.R,
# last re-saved by its 04_group_comparison.R). The values agree; the panel now
# reads the object the verification pass reads.
...
stopifnot(nrow(cg) == 8774L,
          all(c("gene", "ens", "z_resid", "pct_z", "lfc_6W", "lfc_12W",
                "in_ranking") %in% names(cg)),
...
# interaction p-values and the two temporal contrasts, from interaction_results.rds
# (the object script 54 reads), keyed by the scan's own Ensembl IDs; the mapping is
# checked against the scan's own fold changes rather than trusted
int <- as.data.frame(ir$interaction_raw)
MARK$ens <- cg$ens[match(MARK$gene, cg$gene)]
MARK$int_p    <- int$pvalue[match(MARK$ens, rownames(int))]
MARK$int_padj <- int$padj[match(MARK$ens, rownames(int))]
tlfc <- function(g, contrast)
  as.data.frame(ir[[contrast]])[cg$ens[match(g, cg$gene)], "log2FoldChange"]
...
LEGEND <- panel_legend(
  ...
  detail = c(
    ...
    sprintf("THE SAME DEPARTURE IS REACHED BY TWO DIFFERENT ROUTES, and the text should keep the distinction. Foxo3: Myc raises it at six weeks (%+.3f) and lowers it at twelve (%+.3f), and the WILD-TYPE gland raises it with age (%+.3f) while the Myc+ gland does not (%+.3f). Bbc3: %+.3f to %+.3f, with the wild-type gland flat (%+.3f) and the Myc+ gland falling (%+.3f).",
            ...
            tlfc("Bbc3", "timepoint_neg_raw"), tlfc("Bbc3", "timepoint_pos_raw")),
