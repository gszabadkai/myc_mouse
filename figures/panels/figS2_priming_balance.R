...
LEGEND <- panel_legend(
  ...
  detail = c(
    sprintf("n = 6 wild-type animals per timepoint, %d transcripts. DESeq2 raw (unshrunken) log2 fold changes on the `6>12W_wt` contrast; adjusted p-values are IHW (independent hypothesis weighting, a weighted Benjamini-Hochberg), genome-wide.",
            nrow(d)),
    sprintf("%d of %d transcripts reaches padj < 0.05, and it moves the WRONG WAY for a gland moving away from apoptosis: %s %+.2f (padj %.3f), a pro-apoptotic gene going UP. Nothing else clears 0.05 on either arm or among the brakes.",
            n_sig_wt, nrow(d), one$gene[1], one$lfc[1], one$padj[1]),
    sprintf("THE PRO- TO ANTI-APOPTOTIC BALANCE IS A DIFFERENCE, and both arms move the same way, so the balance does not move. On the content ruler the pro arm shifts %+.4f and the anti arm %+.4f across the window, a difference of %+.4f; on the content-blind mitoPPS ruler %+.4f and %+.4f, a difference of %+.4f. (Script 40's ruler, the same instrument as Fig. 2F.)",
  ...
  bounds = c(
    ...
    "A NEGATIVE ON TRANSCRIPTS IS NOT A NEGATIVE ON THE APOPTOTIC THRESHOLD. How close a mitochondrion sits to that threshold is a property of its protein complement; transcript levels are a substrate for it, not a measurement of it. The measurement is BH3 profiling, and this panel is the reason to do it rather than a substitute.",
    "THE CIRCULARITY WARNING OF SCRIPT 34 DOES NOT APPLY HERE, and it should not be imported. `MITOCARTA_APOPTOSIS_PRO`/`_ANTI` are MitoCarta sets, so a coupling between a pro:anti composite and a mitochondrial axis is mito-vs-mito and circular. This panel is a temporal contrast on the transcripts themselves; nothing here is correlated with a mitochondrial score.",
  ...
  source = c(
    ...
    "results/interaction_results.rds (scripts/03_deseq_results_qc.R) -- adjusted p-values for the genotype contrast",
    "results/combined_df_annotated_raw.rds (the archived main pipeline: scripts/archive_main_pipeline/02_deseq_interaction_model.R, last re-saved by its 04_group_comparison.R) -- the symbol-to-Ensembl mapping the power control needs",
